import { Article, Category, Author, GalleryItem, Podcast, IssueDay, BreakingNewsItem } from "@/types/alqabas";
...
function buildArticles(): Article[] {
  return rawArticles.map((r) => {
    const cat = categories.find((c) => c.id === r.categoryId) || categories[0];
    const author = r.authorId ? authors.find((a) => a.id === r.authorId) : undefined;
    const now = new Date();
    return {
      id: r.id,
      title: r.title,
      slug: r.title.replace(/[\s:،.]+/g, "-").replace(/-+/g, "-").toLowerCase(),
      summary: r.summary,
      content: `<p>${r.summary}</p><p>تشهد الساحة المحلية تطورات متسارعة في هذا الشأن، حيث أكدت المصادر الرسمية أن الجهات المعنية تتابع الموقف عن كثب واتخذت الإجراءات اللازمة.</p><p>وقالت المصادر إن الكويت تؤكد دائماً على أهمية الحوار والطرق الدبلوماسية في حل الخلافات، مع الاحتفاظ بحقها في الدفاع عن سيادتها وأمنها الوطني وفقاً للقانون الدولي.</p><p>من جانبه، أشاد المراقبون بالجهود التي تبذلها الجهات المختصة في متابعة هذا الملف المهم، مؤكدين أن الكويت كانت وما زالت مثالاً يحتذى به في المنطقة في التعامل مع مثل هذه المواقف بحكمة واقتدار.</p><p>يذكر أن هذه التطورات تأتي في إطار التوترات الإقليمية المستمرة، والتي تتطلب موقفاً موحداً من الدول العربية للحفاظ على أمن واستقرار المنطقة.</p>`,
      excerpt: r.summary,
      ...
      author: author || { id: 0, name: "القبس", title: "محرر", bio: "" },
...
export function getArticlesByAuthor(authorId: number | string): Article[] {
  return articles.filter((a) => Number(a.authorId) === Number(authorId));
}

export function getArticle(idOrSlug: number | string): Article | undefined {
  const slug = typeof idOrSlug === 'string' ? idOrSlug.toLowerCase().normalize('NFC') : String(idOrSlug);
  return articles.find((a) => a.id === idOrSlug || a.slug.toLowerCase().normalize('NFC') === slug);
}
...
export function searchArticles(query: string): Article[] {
  ...
  return articles.filter(
    (a) => a.title.includes(q) || a.summary.includes(q) || a.category.name.includes(q) || a.author.name?.includes(q)
  );
}
...
export function getBreakingNews(): BreakingNewsItem[] {
  return articles
    .filter((a) => (a as unknown as { breakingNews?: boolean }).breakingNews)
    .slice(0, 8)
    .map((a) => ({
      id: String(a.id),
      title: a.title,
